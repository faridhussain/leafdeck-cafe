...
export default function SignatureDishes() {
    return (
        <section className='relative overflow-hidden bg-[#1B1611] py-32 select-none'>
            {/* Ambient glow */}
            <div className='pointer-events-none absolute left-1/2 top-0 h-[34rem] w-[60rem] -translate-x-1/2 rounded-full bg-[#C9A227]/5 blur-[140px]' />
            ...
            <div className='relative z-10 mx-auto w-[92%] max-w-[1800px]'>
                ...
                <div className='grid grid-cols-1 gap-x-8 gap-y-16 sm:grid-cols-2 lg:grid-cols-3'>
                    {dishes.map((dish) => (
                        <div key={dish.name} className='group'>
                            <div className='relative overflow-hidden rounded-[28px] shadow-[0_18px_50px_rgba(0,0,0,0.18)]'>
                                <Image
                                    src={dish.image}
                                    alt={dish.name}
                                    width={800}
                ...
                <div className='mt-24 flex justify-center'>
                    <Link href='/menu' className='group flex items-center gap-2 rounded-full border border-[#F4EDE1]/20 px-9 py-4 text-[13px] font-semibold uppercase tracking-[0.18em] text-[#F4EDE1] transition-all duration-300 hover:border-[#F4EDE1]/20 hover:bg-white/[0.03]'>
                        Explore Full Menu
