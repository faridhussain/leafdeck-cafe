    ...
    return (
        <section id='signature-dishes' className='relative overflow-hidden bg-[#1B1611] py-14 sm:py-18 lg:py-23'>
            ...
            <div className='relative z-10 mx-auto w-[92%] max-w-[1800px]'>
                <motion.div className='mb-8 text-center sm:mb-14 lg:mb-16' initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.7 }}>
                    <h2 className={`${fraunces.className} text-[2rem] font-semibold tracking-[-1px] text-white sm:text-5xl sm:tracking-[-0.03em] lg:text-6xl xl:text-7xl`}>Our Signature Dishes</h2>
                    ...
                    {dishes.map((dish, index) => (
                        <motion.div key={dish.name} variants={itemVariants} className={`group ${index === dishes.length - 1 ? 'col-span-2 mx-auto w-full max-w-42.5 sm:col-span-1 sm:max-w-none' : ''}`}>
                            ...
                            <div className='mt-2 text-center sm:mt-4'>
                                <h3 className={`${fraunces.className} text-[13px] leading-tight tracking-tight text-white transition-colors duration-300 group-hover:text-[#F4EDE1] sm:text-xl lg:text-2xl`}>{dish.name}</h3>

                                <div className='mx-auto mt-2 h-px w-6 bg-[#C9A227]/45 transition-all duration-500 group-hover:w-12 group-hover:bg-[#D8B45A] sm:mt-3 sm:w-10 sm:group-hover:w-20' />
                            </div>
                        </motion.div>
                    ))}
                </motion.div>
                ...
                >
                    <Link
                        href='/menu'
                        className='group flex w-full max-w-52 sm:max-w-70 items-center justify-center gap-2 rounded-full border border-[#F4EDE1]/20 px-4 py-2.5 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#F4EDE1] transition-all duration-300 hover:bg-white/3 sm:w-auto sm:px-9 sm:py-4 sm:text-[13px] sm:tracking-[0.18em]'
                    >
                        Explore Full Menu
                        <ArrowRight className='sm:h-4 sm:w-4 w-3 h-3 transition-transform duration-300 group-hover:translate-x-1' />
                    </Link>
                </motion.div>
            </div>
        </section>
